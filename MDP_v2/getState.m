...
states(state_1_num * state_2_num, 1) = struct('seq',[],'data',[]);
actions(action_num, 1) = struct('seq',[], 'data',[]);
count = 1;
...
for i = 1:1:length(state_1)
    for j = 1:1:length(state_2)
        states(count).seq = count;
        states(count).data= [state_1(i), state_2(j)];
        count = count +1;
    end
end
count = 1;
for a = action_min: action_gap : action_max
    actions(count) .seq = count;
    actions(count) .data = action_min + action_gap * (count - 1);
    count = count + 1;
end
