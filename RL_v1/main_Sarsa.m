...
%% start Sarsa
sa = Sarsa(states, actions, alpha, gama, epsilon);
try
    for epi = 1:1:episode_num
        env = env.reset();
        current_state = initial_state;
        action_to_take = sa.getEpsilonAction(current_state);
        while 1
            fprintf('-----------------------------------\n');
            fprintf('[step] %d \n', env.step);
            env = env.update();
            ...
            action_to_take_prime = sa.getEpsilonAction(state_prime);
            %for display
            %===========================================
            ...
            fprintf('[Reward] %.2f \n', reward);
            %===========================================
            sa = sa.updateLearn(current_state, state_prime, action_to_take, action_to_take_prime, reward);
            if done
                break;
            end
